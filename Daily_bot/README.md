...
- 예열 시점의 전체 자본을 기준으로 슬롯 수를 먼저 고정하고, 종목당 예산은 `전체자본 / 전체슬롯수`로 고정합니다.
- 예를 들어 `1200만 원 -> 3슬롯 -> 슬롯당 400만 원`, `2000만 원 -> 4슬롯 -> 슬롯당 500만 원`으로 하루 동안 유지됩니다.
- 종목당 최대 투입 금액은 `max_budget_per_stock_krw`로 한 번 더 제한할 수 있으며, 현재 기본값은 500만 원입니다.
- `target_budget_ratio_per_stock`는 `slot_budget_unit_krw`가 0일 때만 사용하는 fallback 비율입니다.
- `max_position_count: 0`이면 고정 종목 수 제한은 두지 않고, 예열 시점에 확정된 슬롯 수가 사실상 장중 활성 종목 상한으로 동작합니다.
