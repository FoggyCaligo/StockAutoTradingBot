...
def run_backtest_dynamic_expect_stop(
    db_path: Path,
    ...
    """Replay the normal strategy, adding one backtest-only exit rule.

    Every recorded refresh reuses that refresh's newly calculated expect_price.
    If the newly expected sell price falls below the position's entry price, the
    position is exited at the current trace price with exit_reason
    ``negative_expectation_stop``.

    The live bot and the normal replay_market_traces.py behavior are untouched.
    """
    ...
    for session_date, day_rows in grouped_by_session.items():
        ...
        for created_at in sorted(set(rows_by_time) | session_override_times):
            ...
            for ticker, position in list(open_positions.items()):
                ...
                if 0 < refreshed_expected_sell_price < position.entry_price:
                    trades.append(
                        base.BacktestTrade(
                            session_date=session_date,
                            ticker=ticker,
                            ...
                            buy_amount_krw=position.invested_amount,
                            sell_amount_krw=position.quantity * current_price,
                            exit_reason="negative_expectation_stop",
                            pnl_percent=base._realized_pnl_percent(position.entry_price, current_price),
                        )
                    )
                    available_cash += position.quantity * current_price
...
def main() -> None:
    ...
    result = run_backtest_dynamic_expect_stop(
        db_path=resolved_db_path,
    ...
    dynamic_stop_count = sum(trade.exit_reason == "negative_expectation_stop" for trade in result)
    print(f"negative_expectation_stops={dynamic_stop_count}")
