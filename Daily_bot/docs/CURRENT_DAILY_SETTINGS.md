...
각 방향별 호가 레벨 수가 `N`개라면 첫 레벨은 가중치 `1.0`, 마지막 레벨은 정확히 `0.0`이다. 따라서 **가장 먼 호가의 가격 레벨 자체는 남아 있지만 잔량은 0으로 환산되어 균형가격 계산에 사실상 기여하지 않는다.** 이 동작은 현재 의도된 설정이며, `0.05 / 0.10 / 0.20` 최소가중치 스윕 결과와 비교한 뒤 `0.0`을 유지하기로 했다.
...
- `top_ratio = 1.0`
- 전일 등락률 필터 OFF
- orderbook bid/ask 선형 감쇠 `1.0 -> 0.0`
- dynamic stop `-0.1% / 3회`
- 손절 슬롯 당일 폐쇄
- 손절 종목 당일 재진입 금지
- 11:30 신규매수 마감
...
주의: 과거 `replay_market_traces.py`의 CLI 기본값 처리에서 설정값 `0.0`이 Python의 `or 1.0` 때문에 `1.0`으로 바뀌는 버그가 있었다. 현재는 수정되어 **설정의 0.0 감쇠값을 그대로 보존**한다. 따라서 감쇠 관련 비교에서는 버그 수정 이후 결과를 기준으로 본다.
