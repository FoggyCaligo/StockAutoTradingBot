...
$LockPath = Join-Path $LogDir "run_monitor.lock"
...
if (Test-Path $LockPath) {
    if (Test-BotPythonRunning -BotRootPath $BotRoot -EntryPath $MainPath -CommandMarker " monitor ") {
        Write-Output "Weekly_bot monitor is already running. Skip duplicate start."
        exit 0
    }
...
try {
    & $PythonPath main.py monitor --real --data live --log-dir $LogDir
}
finally {
    if (Test-Path $LockPath) {
        Remove-Item $LockPath -Force
