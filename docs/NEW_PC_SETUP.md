...
## 4. Config Check (`config/settings.yaml`)
...
- `scripts/run_real.ps1`
  - runs `main.py --real`
  - writes logs to `logs\run_real_YYYYMMDD_HHMMSS.log`

Register task (run once in elevated or allowed PowerShell):

```powershell
$scriptPath = "C:\work\StockAutoTradingBot\scripts\run_real.ps1"
...
Get-Content .\logs\<LATEST_LOG_FILE>
```
